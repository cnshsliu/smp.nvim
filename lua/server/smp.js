...
const getStructure = function (home, fn) {
	const markdown = fs.readFileSync(fn, 'utf8');
...
const getNavigationDiv = function (home, fn, ispreview = true) {
	const structure = getStructure(home, fn);
	console.log(JSON.stringify(structure, null, 2));
	let links_html = '<h2>Links</h2>';
	for (const link of structure.links) {
		links_html += `<div class="smp_nav_link"><a href="${getZettelPath(link.fullpath)}">${
			link.title
		}</a></div>`;
	}
	let backlinks_html = '<h2>Backlinks</h2>';
	for (const link of structure.backlinks) {
		backlinks_html += `<div class="smp_nav_link"><a href="${getZettelPath(link.fullpath)}">${
			link.title
		}</a></div>`;
	}
	let tags_html = '<h2>Tags</h2>';
	for (const tag of structure.tags) {
		tags_html += `<span class="smp_nav_tag"><a href="${getTagHref(tag, fn)}">${tag}</a></span>`;
	}
	const ret = `
<div id="smp_nav_container" class="${smpConfig.show_navigation_content ? '' : 'noshow'}">
      <div class="smp_section " id="section_links"> ${links_html} </div>
      <div class="smp_section " id="section_backlinks"> ${backlinks_html} </div>
      <div class="smp_section " id="section_tags"> ${tags_html} </div>
</div>
<div id="smp_nav_panel" class="${smpConfig.show_navigation_panel ? '' : 'noshow'}">
  <div><a href="#section_links" onclick="javascript:showSection('links')">Links</a></div>
  <div><a href="#section_backlinks" onclick="javascript:showSection('backlinks')">Backlinks</a></div>
  <div><a href="#section_tags" onclick="javascript:showSection('tags')">Tags</a></div>
  ${
		ispreview
			? ''
			: '<div><a href="#" onclick="javascript:editThis(\'' + fn + '\');">Edit</a></div>'
	}
  <div><a href="#" onclick="javascript:hideNavigation()">Hide</a></div>
</div>
<script>
  function showSection(section_name){
      document.getElementById('smp_nav_container').classList.remove('noshow');
      document.getElementById('smp_nav_top_tags').classList.remove('noshow');
  }
  function hideNavigation(){
      document.getElementById('smp_nav_container').classList.add('noshow');
      document.getElementById('smp_nav_top_tags').classList.add('noshow');
		...
		handler: (request, h) => {
			...
			if (md_cache) {
				...
				const resp_html =
					...
					html +
					getNavigationDiv(smpConfig.home, fn_key) +
					'</article>' +
				...
				if (md_cache) {
					if (md_cache.ts !== Number(ts)) {
						if (md_cache.touched[0]) {
							...
							return h.response({
								code: 'touched_all',
								html: html + getNavigationDiv(smpConfig.home, fn_key),
				...
				if (!isMarkdownFile(fn)) {
					...
					return h.response(
						getStylesheet() +
							'<article class="markdown-body">' +
							html +
							getNavigationDiv(smpConfig.home, fn, false) +
							'</article>' +
							copyright +
							getZettelScript(),
					);
