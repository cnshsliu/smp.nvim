...
const indicator = function(lnr, scroll = true) {
  return `<span class="smpanchor ${scroll ? 'scrollto' : ''}" id="lucas_tkbp_${lnr + 1
    }">&nbsp;</span>`;
};
  ...
  function process_file_taggedLines(file_path, title) {
    ...
    for (let i = 0; i < lines.length; i++) {
      if (lines[i].match(regex_this_tag)) {
        const highlightedLine = lines[i].replace(regex_this_tag, replacement_this_tag);
...
const patchLine = (
  ...
  if (line.match(/^\s*\[.+]:\s*.+$/)) {
    ...
    for (const link of structure.links) {
      links_html += `<span class="smp_nav_link"><a href="${getZettelPath(link.fullpath)}">${link.title
        }</a></span>`;
    }
    links_html += '</div>';
    line = links_html;
    ...
    for (const link of structure.backlinks) {
      backlinks_html += `<span class="smp_nav_backlink"><a href="${getZettelPath(link.fullpath)}">${link.title
        }</a></span>`;
    }
    backlinks_html += '</div>';
    line = backlinks_html;
...
const init = async () => {
  ...
  server.route({
    ...
    handler: (request, h) => {
      ...
      let ret = `Bufnr: ${payload.bufnr}, Pos: ${payload.pos}, Stores: ${Object.keys(string_stores).length
        } `;
