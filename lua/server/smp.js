...
const getZettelScript = function () {
	return `<script>
    function editThis(path){console.log(path);
      fetch("http://127.0.0.1:3030/editThis?path=" + encodeURIComponent(path))
    }
  </script><style>
.edit_button{
position:fixed; bottom: 5px; right: 5px; border: 1px solid; border-radius: 5px; padding: 2px; background-color: #f0f0f0; z-index:100;
}
.edit_button a{
				...
				if (!isMarkdownFile(fn)) {
					...
					return h.response(
						getStylesheet() +
							...
							'</article>' +
							`<center><br/><br/>generated by <a href="https://github.com/cnshsliu/smp.nvim">smp.nvim</a>, created by <a href="https://www.buymeacoffee.com/liukehong">LiuKeHong</a> </center>` +
							`<div class="edit_button"> <a href="#" onclick="javascript:editThis('${fn}')" >Edit in Neovim</a></div>` +
							getZettelScript(),
					);
				}
			} else {
